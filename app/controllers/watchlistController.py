...
def dashboard():
    ...
    return render_template("dashboard.html", total=total,
        status_counts=status_counts, type_counts=type_counts, recent=recent)
...
def view_watchlist():
    ...
    elif sort == "title":    query += " ORDER BY title ASC"
    ...
    return render_template("watchlist.html", titles=titles, search=search,
        filter_type=filter_type, filter_status=filter_status, sort=sort)
...
def add_title():
    if request.method == "POST":
        ...
        if not title or not type_:
            ...
            return render_template("add.html", tmdb_api_key=config.TMDB_API_KEY)
        if len(title) > 200:
        ...
        cursor.execute(
            """INSERT INTO watchlist
               (user_id, title, type, status, genre, year, rating, notes, poster_url)
               VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)""",
            (user_id, title, type_, status,
             genre or None, year or None, rating or None,
             notes or None, poster_url or None),
        )
        conn.commit()
        cursor.close()
        conn.close()
...
def edit_title(id):
    ...
    if request.method == "POST":
        ...
        cursor.execute(
            """UPDATE watchlist
               SET title=%s, type=%s, status=%s, genre=%s,
                   year=%s, rating=%s, notes=%s
               WHERE id=%s AND user_id=%s""",
            (title, type_, status, genre or None,
             year or None, rating or None, notes or None, id, user_id),
        )
        conn.commit()
        cursor.close()
        conn.close()
...
def delete_title(id):
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM watchlist WHERE id = %s AND user_id = %s", (id, user_id))
    conn.commit()
    cursor.close()
    conn.close()
